...
export async function executeLiveOrder(request: LiveOrderRequest): Promise<LiveOrderResult> {
  ...
  const apiKey = decryptSecret(request.secret.encryptedKey, env.encryptionKey);
  const apiSecret = decryptSecret(request.secret.encryptedSecret, env.encryptionKey);
